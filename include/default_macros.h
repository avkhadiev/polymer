// 2017 Artur Avkhadiev
/*! \file default_macros.h
*/
#ifndef POLYMER_DEFAULT_MACROS_H
#define POLYMER_DEFAULT_MACROS_H
#define VERBOSE 1
#define DEBUG 1
#define DEFAULT_NB 0                    // number of bonds in a polymer
#define DEFAULT_M 1.0                   // atomic mass
#define DEFAULT_D 3.0                   // bond length
#define DEFAULT_NSOLVENTS 3             // number of solvent molecules
#define DEFAULT_NM 0                    // number of polymer molecules
#define DEFAULT_SOLVENT_DENSITY 0.7
#define DEFAULT_TEMPERATURE     1.0
#define DEFAULT_NC              4       // number of cells in fcc lattice
#define PI 3.14159265359
#endif
